...
function ParcelPanel({ pin, onClose, onParcelLoaded, mapRef }) {
  ...
  const downloadReport = async () => {
    ...
    try {
      ...
      // ── Togibox authenticity seal ──────────────────────────────────────────
      const seal = fullData.verification_seal;
      if (seal) {
        newPageIfNeeded(130);
        secHeader('TOGIBOX AUTHENTICITY SEAL');
        ...
        doc.text('Issued by  togibox.eth', M + 14, y + 18);
      ...
      for (let p = 1; p <= totalPages; p++) {
        ...
        doc.setFontSize(7.5);
        doc.setTextColor(...MUTED);
        doc.text('Generated by Togibox  |  Chainlink CRE + GIWA', M, PH - 10);
        doc.text(`Page ${p} / ${totalPages}`, PW - M, PH - 10, { align: 'right' });
      }

      doc.save(`Togibox-${pin}-Report.pdf`);
    } catch (err) {
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!parcelData && !loading && !error && (
          <div className="m-5 p-4 rounded-xl text-sm text-gray-400"
            ...
            <div className="text-xs text-gray-500">This parcel has no recorded data in Togibox.</div>
          </div>
        )}
            ...
            <div className="px-5 py-3">
              ...
              <div className="flex items-center gap-3 p-3 rounded-xl"
                ...
                <div>
                  ...
                  <div className="text-[10px] text-gray-500 mt-0.5">TogiboxOracle.sol · GIWA Sepolia</div>
                </div>
              </div>
              {onChain > 0 && (
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b flex-shrink-0"
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="font-black text-white tracking-wide">TOGIBOX AI</span>
          </div>
      ...
      <div
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <a href="/" className="flex items-center gap-2">
            <img src="/togibox-favicon.svg" alt="Togibox" className="h-8 w-8 flex-shrink-0" />
            <div className="hidden sm:block leading-none">
              <div className="text-white font-black text-sm">Togibox</div>
